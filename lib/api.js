...
export async function fetchPosts(page = 1, perPage = 5) {
  try {
    const res = await fetch(
      `http://dfw.local/wp-json/wp/v2/posts?_embed=true&fields=id,title,slug,date,excerpt,content,_embedded&page=${page}&per_page=${perPage}`
    );
...
export async function fetchPostBySlug(slug) {
  try {
    const res = await fetch(`http://dfw.local/wp-json/wp/v2/posts?slug=${slug}`);
...
export const fetchCustomApiData = async () => {
  try {
    const response = await fetch('http://dfw.local/wp-json/custom/v1/coustom-api');
...
export const fetchContactData = async () => {
  try {
    const response = await fetch('http://dfw.local/wp-json/custom/v1/custom-fields');
