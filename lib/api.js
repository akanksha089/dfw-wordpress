...
export async function fetchPosts(page = 1, perPage = 5) {
  try {
    const res = await fetch(
      `http://localhost/wp-json/wp/v2/posts?_embed=true&fields=id,title,slug,date,excerpt,content,_embedded&page=${page}&per_page=${perPage}`
    );
...
export const fetchCustomApiData = async () => {
  try {
    const response = await fetch('http://localhost/wp-json/custom/v1/coustom-api');
...
export const fetchContactData = async () => {
  try {
    const response = await fetch('http://localhost/wp-json/custom/v1/custom-fields');
