...
// Updated fetchPosts function with HTML stripping
// export async function fetchPosts(page = 1, perPage = 5) {
//   try {
//     const res = await fetch(
//       `${BASE_URL}/wp/v2/posts?_embed=true&fields=id,title,slug,date,excerpt,content,_embedded&page=${page}&per_page=${perPage}`
...
//     // Log the raw response text to inspect any issues
//     const rawText = await res.text();  // Read the response as text
//     console.log('Raw response text:', rawText);  // Log the raw response

//     // Now try parsing the JSON manually
//     const posts = JSON.parse(rawText);  // Attempt to parse JSON from the raw response
