...
export async function fetchPosts(page = 1, perPage = 5) {
  try {
    const res = await fetch(
      `${BASE_URL}/wp/v2/posts?_embed=true&page=${page}&per_page=${perPage}`
    );

    console.log('✅ Fetching from:', res.url);

    if (!res.ok) {
      console.warn('❌ Server responded with error code', res.status);
    ...
    let posts = [];
    try {
      posts = await res.json(); // 👈 direct parse, no slicing, no guessing
    } catch (jsonErr) {
      console.warn('❌ Failed to parse JSON:', jsonErr.message);
      return { data: [], totalPages: 0 }; // Fallback safe
    }
    ...
    console.error('🚨 Final catch - fetch failed:', err.message);
    return {
      data: [],
      totalPages: 0,
    };
